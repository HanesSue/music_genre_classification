...
if __name__ == "__main__":
    ...
    # model = TemporalCNN(input_channels=128, num_classes=8, num_filters=64).to(device)
    # model.fc1.apply(_init_weights)
    # model.fc2.apply(_init_weights)
    # model = MultibranchLSTM(num_classes=8, resized=False).to(device)
    model = CRNNwithResNet(num_classes=8, resnet="resnet18").to(device)
    # model = ResNet34Classifier(num_classes=8).to(device)
    ...
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        if "bias" in name or "bn" in name.lower() or "batchnorm" in name.lower():
    ...
    for epoch in range(num_epochs):
        ...
        with open("./checkpoint/log.json", "w", encoding="utf-8") as f:
        ...
        if accuracy > best_val_acc:
            ...
            torch.save(
                model.state_dict(),
                f"./checkpoint/DeepCRNNresnet18inclass_epoch_{epoch+1}_{timestamp}.pth",
            )
        else:
            patience_counter += 1
